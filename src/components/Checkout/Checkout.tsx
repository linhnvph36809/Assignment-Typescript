...
const Checkout = () => {
  ...
  const onSubmit: SubmitHandler<payForm> = async (data) => {
    ...
    try {
      const { data:url } = await pay(trip.price,trip._id,data.name,data.email,data.phone);
  ...
  return (
    <div className="md:w-[730px] mx-auto bg-[#F2F2F2] my-10 rounded-lg px-5 pb-20">
      <div className="flex justify-center py-5 border-b-[2px] border-solid border-[#E0E0E0]">
        <img src="../src/assets/images/logo-pay-3.png" alt="" className="md:w-[250px] [@media(max-width:431px)]:w-[100px]" />
      </div>
      <div className="flex justify-between items-center py-5">
        <div className="flex gap-4">
          <img src="../src/assets/images/detail-1.jpg" alt="" className="md:w-[100px] [@media(max-width:431px)]:w-[80px]" />
          <div className="flex flex-col justify-between">
            <h3 className="md:text-[14px] font-semibold [@media(max-width:431px)]:text-[12px]">{trip.busHouseId?.name}</h3>
            <div>
        ...
        <div>
          ...
          <h3 className="lg:text-base text-[#2474E5] font-semibold text-end [@media(max-width:431px)]:text-[12px]">Tạm tính: {trip.price}đ</h3>
        </div>
      </div>
      <form action="" onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className="grid md:grid-cols-2 gap-x-5">
          <div className="pt-4">
            <label htmlFor="" className="block pb-2 md:text-sm font-medium text-[#484848] [@media(max-width:431px)]:text-xs">Tên người dùng</label>
            <input type="text" {
              ...register("name",{ required: "Vui lòng nhập tên" })
              } 
            className="w-full h-[42px] rounded-md px-5 border-[1px] border-solid border-[#ccc]" />
          {errors.name?.message && <p className="my-2 text-red-500 text-xs" >{errors.name?.message}</p>}
          </div>
          <div className="pt-4">
          ...
          <div className="pt-4">
            <label htmlFor="" className="block pb-2 md:text-sm font-medium text-[#484848] [@media(max-width:431px)]:text-xs">Nội dung</label>
            <input type="text" disabled value="Thanh toan ve xe" className="w-full h-[42px] rounded-md px-5 border-[1px] border-solid border-[#ccc] text-[#2474E5]" />
          </div>
        </div>
        <div className="flex [@media(max-width:431px)]:flex-col-reverse py-5 items-start justify-between border-b-2 border-solid border-[#E0E0E0]">
          <div className="[@media(max-width:431px)]:w-full">
            ...
            <div className="flex gap-2 items-center mb-2">
              ...
              <h3 className="flex items-center gap-2 text-xs"><img src="../src/assets/images/logo-pay.png" alt="" className="w-[30px]" />Thanh toán VNPAY</h3>
            </div>
          </div>
          <div className="[@media(max-width:431px)]:w-full">
            ...
            <div className="flex justify-between items-center gap-5">
              ...
              <h3 className="lg:text-lg text-[#2474E5] font-semibold text-end [@media(max-width:431px)]:text-[14px]">{trip.price}đ</h3>
            </div>
          </div>
        </div>
        <div className="flex [@media(max-width:431px)]:flex-col-reverse justify-between items-center py-5">
          ...
          <button type="submit" className="md:w-1/3 bg-[#FFC700] py-3 font-semibold text-base rounded-lg [@media(max-width:431px)]:w-full [@media(max-width:431px)]:my-3 [@media(max-width:431px)]:py-2 [@media(max-width:431px)]:rounded-md" >Đặt vé</button>
        </div>
      </form>
    </div>
  )
